...
namespace backend.services.Controllers.Maestros
{
    ...
    public class DireccionController : ControllerBase
    {
        ...
        [HttpPost("[action]")]
        public async Task<ActionResult<ApiResponse<SqlRspDTO>>> postUpdDireccionPrincipal([FromBody] DireccionDTO direccion)
        {
            ApiResponse<SqlRspDTO> response = new ApiResponse<SqlRspDTO>();

            try
            {
                var result = await service.UpdDireccionPrincipal(direccion);
