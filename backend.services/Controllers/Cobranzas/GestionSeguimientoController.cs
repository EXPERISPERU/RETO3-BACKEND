...
namespace backend.services.Controllers.Cobranzas
{
    ...
    public class GestionSeguimientoController : ControllerBase
    {
        ...
        [HttpGet("[action]")]
        public async Task<ActionResult<ApiResponse<List<SelectDTO>>>> getSelectAsesorSeguimiento(int nIdCompania, int nIdUsuario, int tipoListSeguimiento)
        {
            ApiResponse<List<SelectDTO>> response = new ApiResponse<List<SelectDTO>>();

            try
            {
                var result = await service.getSelectAsesorSeguimiento(nIdCompania, nIdUsuario, tipoListSeguimiento);
        ...
        [HttpPost("[action]")]
        public async Task<ActionResult<ApiResponse<SqlRspDTO>>> postInsDescartarReferido([FromBody] DescartarReferidoDTO descartarRef)
        {
            ApiResponse<SqlRspDTO> response = new ApiResponse<SqlRspDTO>();

            try
            {
                var result = await service.InsDescartarReferido(descartarRef);
